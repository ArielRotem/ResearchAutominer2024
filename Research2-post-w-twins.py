...
def main():
    ...
    over_38_data = remove_rows_below_threshold(de_dupe_data, 'fever temperature numeric_max 37.5-43 after delivery-numeric result', 38)
    ...
    data = containswords_andor_containswords_result_exists(below_threshold_data, 'cultures-test type_1', ['דם'], 'OR','cultures-specimen material_1', ['דם'], 8, 104, 'blood_culture_taken')
    ...
    data = update_column_with_values_batch(data, 'surgery before delivery-procedure', words_dict_13, default_value="Other", empty_value="0", batch=4)
    #update_column_with_values(data, 'surgery before delivery-procedure_2', words_dict_13, default_value="Other", empty_value="0")
    data = update_column_with_values_batch(data, 'surgery after delivery-procedure', words_dict_13, default_value="Other", empty_value="0", batch=10)
  ...
  # Check if numeric values in column 'gestational age' is in-range or out-of-range for prematurity categories, and add results in a new column '___'
    ...
    data = cutoff_number(data, 'fever temperature numeric_max 37.5-43 before delivery-numeric result', 'fever_before_delivery', 38, above=1, below=0, empty_value='')                     
    ...
    data = flip_sign(data, 'fever temperature numeric_max 37.5-43 before delivery-date of measurement-hours from reference')

    data = combine_columns(data, ['surgery before delivery-procedure_1', 'surgery before delivery-procedure_2', 'surgery before delivery-procedure_3', 'surgery before delivery-procedure_4'], 'Surgery_before_delivery', delimiter=', ')
    data = combine_columns(data, ['surgery after delivery-procedure_1', 'surgery after delivery-procedure_2', 'surgery after delivery-procedure_3', 'surgery after delivery-procedure_4', 'surgery after delivery-procedure_5', 'surgery after delivery-procedure_6', 'surgery after delivery-procedure_7', 'surgery after delivery-procedure_7', 'surgery after delivery-procedure_8', 'surgery after delivery-procedure_9', 'surgery after delivery-procedure_10'], 'Surgery_after_delivery', delimiter=', ')
    ...
    data = time_to_treatment_after_event(
        ...
        num_abx=103,
        step_size=3,
        result_col='time_from_postpartum_fever_treatment_hours'
    )
   ...
   # data = flag_antibiotic_within_timeframe_idx(
    ...
    data = flag_antibiotic_within_timeframe_idx(
        data, 'onset of fever 38 after delivery-date of measurement', 'antibiotics-medication_1', 'antibiotics-date administered _1', 103, 3, 'birth-date of first documentation - birth occurence', 'GBS_prophylactic_treatment_yes/no', antibiotics_to_include=['PENICILLIN G SODIUM', 'CLINDAMYCIN HCL', 'CLINDAMYCIN PHOSPHATE']
    )
